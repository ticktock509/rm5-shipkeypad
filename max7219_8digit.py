...
CUSTOM_MAP = {
    ' ': 0x00,'!':0x02, '@':0x04, '#':0x40, '$':0x01, '%':0x08, '^':0x20, '&':0x10, '.': 0x80
    }
...
class Display:
    ...
    def specialwrite(self, s):
        l = len(s)
        if l < 8:
            s = "%-8s" % s
        for i in range(0,8):
            self.buffer[7-i] = self.decode_char2(s[i])
            
    def write_with_dots(self, s):
      len_s = len(s)
      x = 0
      i = 0
      while i < len_s:
